...
BASE_PATH = '/mnt/dog/chinmay/ms_500/achieva/combined'
...
class FullVolDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return volume, augmented_volume, patient_label
    ...
    def generate_concatenated_scan_volume(self, patient_name):
        ...
        :return: None
        """
        flair_filename = glob.glob(f"{BASE_PATH}/{patient_name}/**/*mni_flair.nii.gz")[0]
        t1_filename = glob.glob(f"{BASE_PATH}/{patient_name}/**/*mni_t1.nii.gz")[0]
        ...
        np.save(file=os.path.join(BASE_PATH, f'{patient_name}_scan.npy'), arr=volume)

    def load_volume(self, patient_name):
        return torch.as_tensor(np.load(file=os.path.join(BASE_PATH, f'{patient_name}_scan.npy')), dtype=torch.float)
