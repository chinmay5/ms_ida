...
class AbstractDataset(Dataset):
    def __init__(self):
        ...
        self.random_node_translation = get_configurations_dtype_boolean(section='TRAINING', key='NODE_TRANS')
        if use_2y:
...
class HeterogeneousNodeLevelPatientDataset(AbstractDataset):
    ...
    def handle_isolated_edges(self, heterogeneous_graph):
        ...
        if 'edge_index' in heterogeneous_graph['lesion']:
            ...
            heterogeneous_graph[('lesion', 'Intercluster', 'lesion')].edge_index = torch.tensor([[], []],
                                                                                                dtype=torch.long)
...
if __name__ == '__main__':
    ...
    print(
        f"Min x: {x_y_z_coord_aug[:, 0].min()}, Min y: {x_y_z_coord_aug[:, 1].min()} and Min z: {x_y_z_coord_aug[:, 2].min()}")
    print(
        f"Max x: {x_y_z_coord_aug[:, 0].max()}, Max y: {x_y_z_coord_aug[:, 1].max()} and Max z: {x_y_z_coord_aug[:, 2].max()}")
